...
export default function LayoutEcosystem(props: layoutEcosystemProps){
    ...
    async function onClickTopic(topic: string){
        ...
        Router.push(`/?topics=${[...selectedEcosystems, topic]
            .filter(n => n != props.ecosystem).toString()}`, {scroll: false});
    ...
    async function removeSubEcosystem(subEcosystem: string){
        ...
        var filteredList = updatedList.filter(n => n != props.ecosystem)
        if(filteredList.length > 0) {
            Router.push(`/?topics=${filteredList}`, {scroll: false});
        }
        else
